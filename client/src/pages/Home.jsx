...
const MODE_LABEL  = { td:'🏰 TD', vs:'⚔️ VS', time_attack:'⏱️ TA', pve:'🤖 PvE' };
...
export function Home() {
  ...
  const isTD = selectedMap && !isVS && !isTA;
  ...
  const startMP = async () => {
    ...
    setError('');
    try {
      const subMode = selectedMap.game_mode === 'td' ? 'coop'
        ...
        {selectedMap && (
          <div style={{
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
              <button onClick={startSolo} disabled={starting} style={{
