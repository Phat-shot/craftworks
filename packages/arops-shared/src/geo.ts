// ═══════════════════════════════════════════════════════════
//  AR OPS — geo math primitives
//  Pure functions, spherical earth model (mean radius).
//  Polygon operations use a local equirectangular projection,
//  which is accurate to <0.1% for playfields up to a few km.
// ═══════════════════════════════════════════════════════════
...
// ── Local planar projection (for polygon math on small areas) ──

export interface XY { x: number; y: number; }

/** Project a point to a local ENU-style plane centered on `origin` (meters). */
export function toLocalXY(p: LatLon, origin: LatLon): XY {
  return {
    x: (p.lon - origin.lon) * DEG2RAD * EARTH_RADIUS_M * Math.cos(origin.lat * DEG2RAD),
...
export function pointInPolygon(point: LatLon, polygon: LatLon[]): boolean {
  ...
  for (let i = 0; i < poly.length; i++) {
    ...
    if (pointSegmentDistance(pt, a, b) < EPS) return true;
  }
...
export function distanceToPolygonEdgeM(point: LatLon, polygon: LatLon[]): number {
...
function onSegment(p: XY, q: XY, r: XY): boolean {
  return (
...
export function segmentsIntersect(p1: XY, q1: XY, p2: XY, q2: XY): boolean {
